...
class prospective_time_forager(nn.Module):
    def __init__(
        self,
        n_features: int,
        n_actions: int,
        ...
        super(prospective_time_forager, self).__init__()
        ...
        # register the precomputed positional encoding vector
        pdb.set_trace()
        pe = self.get_freq_encoding()
        self.register_buffer('pe',pe)

        # Shared hidden layer
        self.shared_fc = nn.Linear(n_features, 128).to(self.device) 
        
        # actor and critic
        self.actor = nn.Linear(128, n_actions).to(self.device)
        self.critic = nn.Linear(128, 1).to(self.device)
    ...
    def get_freq_encoding(self): 
        position = torch.arange(self.max_len,device=self.device).unsqueeze(1)
        div_term = 2 * math.pi / torch.arange(2,self.d_model + 1, 2,device=self.device) 
        ffe = torch.zeros(1, self.max_len, self.d_model,device=self.device) 
    ...
    def time_encoder(self,t):
        ...
        enc = self.pe[:, t.squeeze().long(), :]
        return enc
    
    def forward(self, u, t):
        u = u.to(self.device,  dtype=torch.float32) if isinstance(u, torch.Tensor) else torch.tensor(u, device=self.device, dtype=torch.float32)
        
        t = self.time_encoder(t)
        # concatenate time and state information
    ...
    def select_action(self,u, t):
        action_logits, state_values = self.forward(u,t)
    ...
    def get_future_reward(
        ...
        for iteration in range(10):
            ...
            abs_time_trajecotry = abs_time

            for step in range(horizon):
                
                actions, _, _, _ = self.select_action(states,abs_time_trajecotry)
...
def main(session_duration,
    ...
    obs_shape = 3 * history_length + d_model # reward_length + action_length + location_length + time_embedding length
    ...
    for n_rep in tqdm(range(n_reps)):
        # init the agent
        agent = prospective_time_forager(obs_shape, 
                                              action_shape, 
                                              device, 
                                              critic_lr, 
                                              actor_lr,  
        ...
        for sample_phase in tqdm(range(n_updates)):
            ...
            for step in range(n_steps_per_update):
                ...
                location, rewards, terminated, truncated, infos = env.step(
                    actions.cpu().numpy()
                )
    ...
    with open(f'/results/prospective_forager_w_time.pkl', 'wb') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_steps_per_update", type=int, default=100, help="Number of steps per policy update")
    ...
    parser.add_argument("--d_model", type=int, default=128, help="Critic learning rate")
